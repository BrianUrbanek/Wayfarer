...
import { buildIslandEvidenceConstellation, buildIslandRatingTimelineRows } from '../../model/islandEvidenceVisualization.js';
...
describe('island evidence visualization helper', () => {
  ...
  it('builds constellation points using strongest cohort weight and polarity', () => {
    ...
    assert.equal(model.points[0]?.primaryCohortId, 'c-a');
    assert.equal(model.points[0]?.rating, -1);
  });
});
