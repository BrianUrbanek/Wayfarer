...
describe('recommendation scoring', () => {
  it('only includes unrated islands', () => {
    ...
    expect(recommendations.recommendations.every((entry) => user.ratings[entry.islandId] === null)).toBe(true);
  });
  ...
  it('raises score when predicted fit is stronger', () => {
    ...
    expect(stronger?.recommendationScore ?? 0).toBeGreaterThan(weaker?.recommendationScore ?? 0);
  });
