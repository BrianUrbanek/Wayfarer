  ...
  return (
    <div className="stack">
      ...
      <div className="metric-grid metric-grid--compact">
        <MetricCard label="Behavior match strength" value={selectedInference.behaviorMatchStrength.toFixed(3)} helper="Strongest raw positive cohort fit before the distribution is normalized." />
        <MetricCard label="Behavior specificity" value={selectedInference.behaviorSpecificity.toFixed(3)} helper="How much the behavior distribution prefers one cohort over the runner-up." />
        <MetricCard label="Target agreement" value={selectedInference.targetAlignment.ratedCount > 0 ? `${selectedInference.targetAlignment.agreementCount}/${selectedInference.targetAlignment.ratedCount}` : 'No evidence'} helper={selectedInference.targetAlignment.cohortId ? `${Math.round(selectedInference.targetAlignment.agreementRate * 100)}% agreement with ${cohortLabel(selectedInference.targetAlignment.cohortId)} reference ratings.` : 'No reference cohort available for direct target agreement yet.'} tone={selectedInference.targetAlignment.agreementRate >= 0.8 ? 'success' : selectedInference.targetAlignment.agreementRate >= 0.5 ? 'accent' : 'warning'} />
        <MetricCard label="Cohort separability" value={`${Math.round(selectedInference.cohortSeparability.topGap * 100)} pts`} helper={selectedInference.cohortSeparability.message} tone={selectedInference.cohortSeparability.label === 'high' ? 'success' : selectedInference.cohortSeparability.label === 'moderate' ? 'accent' : 'warning'} />
      </div>
      <div className="report-section">
        <div className="report-section__column"><div className="section-heading"><h3>Declared Tag Distribution</h3><p>Top cohort: {cohortLabel(selectedInference.declaredTop.cohortId)}</p></div>{declaredDistributionChart}</div>
        <div className="report-section__column"><div className="section-heading"><h3>Observed Behavior Distribution</h3><p>{behaviorReadText}</p></div>{behaviorDistributionChart}</div>
      </div>
      ...
      <section className="detail-block">
        <div className="section-heading"><h4>Rater signal profile</h4><p>Cohort-local signal only. No tag-level trust weights are calculated here.</p></div>
        <div className="metric-grid metric-grid--compact">
        ...
        <ReportTable columns={signalColumns} rows={signalRows} getRowKey={(row) => row.cohort.id} emptyTitle="No signal profile" emptyDescription="This user has not yet accumulated enough overlap to form cohort-local signal." />
      </section>
